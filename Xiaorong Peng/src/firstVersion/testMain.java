...
public class testMain {
	...
	public static void main(String[] args) {
		...
		int cost = 0;
		int numBig = (int) (totalVol / volBigContainer);
		int numSmall = 0;
		if (totalVol % volBigContainer <= volSmallContainer) {
			numSmall++;
		}else {
			numBig++;
		}
		...
		if (numSmall > 0) {
			...
			double x = (totalVol % volBigContainer) * den;
			if (x > 500) {
				cost += 1200;
			}else {
				cost += 1000;
		...
		System.out.println("The optimal solution is " + cost + " Euro");
		System.out.println("The number of big container: " + numBig);
		System.out.println("The number of small container: " + numSmall);
